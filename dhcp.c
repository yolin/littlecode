...
int dhcp_new(struct dhcp_t **pdhcp, int numconn, int hashsize,
  ...
  if(_options.bridgemode)
  {
     ...
     {
        log_err(errno, "socket failed for dhcp relay!");
        return -1;
     }
...
{ 0x28, 0, 0, 0x0000000c },
{ 0x15, 17, 0, 0x000086dd },
{ 0x15, 0, 16, 0x00000800 },
{ 0x30, 0, 0, 0x00000017 },
{ 0x15, 14, 0, 0x00000084 },
{ 0x15, 13, 0, 0x00000006 },
...
//tcpdump -i br-lan -dd -s 2048 '((port 67 or port 68) and (udp[8:1] = 0x2))'
...
setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, &Filter, sizeof(Filter));
    ...
    /* if((setsockopt(fd,SOL_SOCKET,SO_BROADCAST, &bd,sizeof bd)) == -1) */
    /* { */
    /*     perror("setsockopt - SO_SOCKET "); */
    /*     return -1; */
    /* } */

  log_dbg("----------dhcp_new  fd[%d]", fd);

  /* char *opt; */
  /* opt = _options.dhcpif; */
  /*   if(setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, opt, strlen(_options.dhcpif)) < 0) */
  /*   { */
  /*       log_dbg("ERROR!!!!!!!!!!!!!!!!!!!"); */
  /*   } */
    ...
    /* char *opt; */
    /* opt = "br-lan"; */
    /* setsockopt(fd, SOL_SOCKET, SO_BINDTODEVICE, opt, 6); */
  }
  else
  {
...
static int dhcp_relay(struct dhcp_t *this, 
  ...
  struct sockaddr_in addr;
  /* struct in_addr bdaddr; */
  /* int broadcast=1; */
  int i=0;
  /* int one = 1; */
  /* const int *val = &one; */
  struct ifreq ifr;
  u_int8_t src_mac[6] = {0};
  ...
  if(_options.bridgemode) {
    ...
    if (dhcp_gettag(pack_dhcp, ntohs(pkt_udphdr(pack)->len)-PKT_UDP_HLEN, &message_type, 
    ...
    if(message_type->v[0] == DHCPREQUEST)
    {
        ...
        if (dhcp_gettag(pack_dhcp, ntohs(pack_udph->len)-PKT_UDP_HLEN, 
                    &requested_ip, DHCP_OPTION_REQUESTED_IP) == -1)
        { 
            memcpy(fullpack, pack, len);
            pack = fullpack;
            pack_ethh = pkt_ethhdr(pack);
            pack_iph = pkt_iphdr(pack);
            pack_udph = pkt_udphdr(pack);
            pack_dhcp = pkt_dhcppkt(pack);
            ep = len- 1;
            pack[ep++] = DHCP_OPTION_REQUESTED_IP;
            pack[ep++] = 0x4;
            memcpy(&pack[ep], &(pack_iph->saddr), 4);
            ep += 4;
            pack[ep++] = DHCP_OPTION_END;
            len += 6;
            pack_udph->len = htons(ntohs(pack_udph->len) + 6);
        }
    }
    ...
    for(i=0;i<6;++i)
    {
        pack_ethh->src[i] = src_mac[i];
        pack_ethh->dst[i] = 0xff;
    }
    pack_ethh->prot = htons(0x0800);
    /* inet_pton(AF_INET, "255.255.255.255", (void *)&bdaddr); */
    /* addr.sin_addr.s_addr = bdaddr.s_addr; */
    /* if((setsockopt(this->relayfd, SOL_SOCKET, SO_BROADCAST, */
    ...
    pack_iph->version_ihl = PKT_IP_VER_HLEN;
    pack_iph->tos = 0;
    pack_iph->tot_len = htons(PKT_IP_HLEN + ntohs(pack_udph->len));
    pack_iph->id = 0;
    pack_iph->ttl = 0x80;
    pack_iph->protocol = 0x11; // UDP(17)
    pack_iph->check = 0;
    pack_iph->saddr = inet_addr("0.0.0.0");//_options.dhcprelayip.s_addr?_options.dhcprelayip.s_addr:_options.uamlisten.s_addr; //br-lan IP
    pack_iph->daddr = inet_addr("255.255.255.255");
    pack_iph->opt_off_high = 0;
    pack_iph->off_low = 0;
    pack_udph->src = ntohs(68);
    pack_dhcp->flags[0] = 0x80; //set to broadcast
    pack_udph->check = 0;

#if 0 //some dhcp server not reply offer if giaddr != 0.0.0.0
    if (_options.dhcprelayip.s_addr)
        pack_dhcp->giaddr = _options.dhcprelayip.s_addr;
    else
        pack_dhcp->giaddr = _options.uamlisten.s_addr;
#endif
    pack_dhcp->giaddr = inet_addr("0.0.0.0");

    log_dbg("Sending DHCP packet to 255.255.255.255");
  }
  else
  {
...
if(_options.bridgemode)
{
    ...
    /* if(setsockopt(this->relayfd, IPPROTO_IP, IP_HDRINCL, val, sizeof(one)) < 0) */
    /* { */
    /*     log_err(errno, "could not set relay socket"); */
    /*     return -1; */
    /* } */

    memset((void*)&socket_address, 0, sizeof(struct sockaddr_ll));
    safe_strncpy(ifr.ifr_name, "br-lan", sizeof(ifr.ifr_name));
    if (ioctl(this->relayfd, SIOCGIFINDEX, (caddr_t)&ifr) < 0) {
    ...
    if (sendto(this->relayfd, pack_ethh, PKT_IP_HLEN + ntohs(pack_udph->len) + PKT_ETH_HLEN, 0, (struct sockaddr*)&socket_address, sizeof(struct sockaddr_ll)) < 0)
    /* if(sendto(this->relayfd, pack_iph, ntohs(pack_iph->tot_len), 0, (struct sockaddr *)&addr, sizeof(addr)) < 0) */
        // Verify
    {
...
static int dhcp_runscript(struct dhcp_conn_t *conn, char* script, int wait) {
  pid_t pid;
  ...
  set_env("RUNSCRIPT", VAL_STRING, "dhcp", 0);
#ifdef ENABLE_LAYER3
  if (_options.layer3)
  ...
  set_env("AUTHENTICATED", VAL_STRING, conn->authstate == DHCP_AUTH_PASS ? "1" : "0", 0);
  set_env("CALLING_STATION_ID", VAL_MAC_ADDR, conn->hismac, 0);
...
int dhcp_getreq(struct dhcp_ctx *ctx, uint8_t *pack, size_t len) {
  ...
  if (this->relayfd > 0) {
    if (_options.macup) {
        log_dbg("Calling dhcp MAC up script: %s",_options.macup);
        dhcp_runscript(conn, _options.macup, 1);
    }
...
int dhcp_set_addrs(struct dhcp_conn_t *conn, 
  ...
  if(_options.bridgemode &&
    (hisip->s_addr & ourmask->s_addr) == (_options.mask.s_addr & _options.net.s_addr) &&
    memcmp(&hisip->s_addr, &_options.dhcpgwip.s_addr, 4) &&
    memcmp(&hisip->s_addr, &_options.ezmasterip.s_addr, 4)) {
      log_dbg("[BridgeMode] dhcp_set_addrs %s dev %s", inet_ntoa(*hisip), _options.dhcpif);
  }
  else {
...
if(_options.bridgemode)
{
    if ((length = recvfrom(this->relayfd, &pack, sizeof(pack), 0,
                         (struct sockaddr *) &addr, &fromlen)) <= 0) {
       log_err(errno, "recvfrom() failed");
       return -1;
    }

    if (length < 44 + PKT_ETH_HLEN + PKT_IP_HLEN + PKT_UDP_HLEN) {
        return -1;
    }
    ...
    if(!memcmp(&pack[7], &this->rawif[0].hwaddr[1], 5))
    {
        /* log_dbg("ignore my send"); */
        return -1;
    }
    length = length - PKT_IP_HLEN - PKT_UDP_HLEN - PKT_ETH_HLEN;
    memcpy(&packet, &pack[PKT_ETH_HLEN+PKT_IP_HLEN+PKT_UDP_HLEN], length);
}
else
{
...
if (!_options.bridgemode)
{
  ...
  if (dhcp_hashget(this, &conn, packet.chaddr)) {
    ...
    if (dhcp_newconn(this, &conn, packet.chaddr)) {
  ...
  else if (_options.bridgemode) {
      struct app_conn_t *occupy_appconn = 0;
      ...
      if (packet.ciaddr) offerip.s_addr = packet.ciaddr;
      if (packet.yiaddr) offerip.s_addr = packet.yiaddr;
          ...
          case DHCPACK:
              ...
              if (!ippool_getip(ippool, &occupy_ipm, &offerip)) {
                  if (occupy_ipm && occupy_ipm->in_use) {
                      if((occupy_appconn = (struct app_conn_t *)occupy_ipm->peer) != NULL ) {
                          if(memcmp(occupy_appconn->hismac, conn->hismac, PKT_ETH_ALEN)) {
                              log_dbg("[BridgeMode] occupy mac: %.2X-%.2X-%.2X-%.2X-%.2X-%.2X", 
                                      occupy_appconn->hismac[0], occupy_appconn->hismac[1], occupy_appconn->hismac[2],
                                      occupy_appconn->hismac[3], occupy_appconn->hismac[4], occupy_appconn->hismac[5]);
                              log_dbg("[BridgeMode] free ip %s", inet_ntoa(occupy_ipm->addr));
                              if (ippool_freeip(ippool, occupy_ipm)) {
                                  log_dbg("[BridgeMode] free ip failure");
                              }
                              dhcp_release_mac(this, occupy_appconn->hismac, RADIUS_TERMINATE_CAUSE_LOST_CARRIER);
                          }
                      }
                  }
              }
              ...
              if (appconn) {
                  ...
                  struct ippoolm_t *ipm = (struct ippoolm_t*)appconn->uplink;
                  if(ipm) {
                      if(offerip.s_addr != conn->hisip.s_addr) {
#ifdef HAVE_NETFILTER_COOVA
                          if (_options.kname) {
                              log_dbg("[BridgeMode] kmod_coova_release hisip %s", inet_ntoa(conn->hisip));
                              kmod_coova_release(conn);
                          }
#endif
                          if (ipm && ipm->in_use) {
                              struct in_addr mask1, gw;
                              int res1;
                              mask1.s_addr = 0xffffffff;
                              gw.s_addr = 0;
                              res1 = net_route(&ipm->addr, &gw, &mask1, _options.dhcpif, 1);
                              log_dbg("[BridgeMode] Removing route for %s gw 0.0.0.0 %d", inet_ntoa(ipm->addr), res1);
                          }
                          ...
                          appconn->uplink=0;
                          this->cb_request(conn, &offerip, 0, 0);
                      }
                  }
              }
              break;
          case DHCPNAK:
              log_dbg("[BridgeMode] recv DHCPNAK hisip %s", inet_ntoa(conn->hisip));
              if (appconn) {
                  appconn->hisip.s_addr=0;
              }
              break;
...

  {
    ...
    if(_options.bridgemode)
    {
        // FIXME: this is gateway ip, change to dhcp ip
  ...
  if (!_options.uamanyip) {
    ...
    */
	...
	log_dbg("ARP: Asked for target: %s", inet_ntoa(taraddr));
      ...
      if(_options.bridgemode)
      {
          log_dbg("[BridgeMode] ARP: response our mac for all arp");
      }
      else
      {
